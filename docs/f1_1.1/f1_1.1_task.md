...
- [x] `[TSK-F1_1.1-18.1-RED]` **[VALIDAR]**: Test pgTap: Snapshotting de parámetros (inmovilidad ante cambios en system_config).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si una variable de sesión cambia tras haber sido capturada al inicio.
- [x] `[TSK-F1_1.1-18.2-RED]` **[VALIDAR]**: Test pgTap: Resolución de conflictos de promoción (Admin > Scraper).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si un resultado de Scraper sobreescribe un registro manual de Admin.
- [x] `[TSK-F1_1.1-18.3-RED]` **[VALIDAR]**: Test pgTap: Bloqueo de promoción ante discrepancia activa (`is_conflict=TRUE`).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si se promociona un sorteo con advertencia de conflicto sin resolver.
- [x] `[TSK-F1_1.1-18.4-RED]` **[VALIDAR]**: Test pgTap: Atomicidad de transacciones ante fallos parciales (Savepoint Trigger).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si una transacción deja estados inconsistentes tras un error forzado.
- [x] `[TSK-F1_1.1-18.5-RED]` **[VALIDAR]**: Test pgTap: Integridad de Backup Forense (Falla si el backup en `system_logs` no se completa).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si la tabla de logs no contiene el snapshot JSONB previo al recálculo.
- [x] `[TSK-F1_1.1-18.6-RED]` **[VALIDAR]**: Test pgTap: Simulación de cambio de configuración `mid-flight` (Verificar persistencia de Snapshot).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si el motor de scoring usa un valor nuevo inyectado a mitad del proceso.
- [x] `[TSK-F1_1.1-19.1-GREEN]` **[EJECUTAR]**: Implementación de `fn_compute_async_scoring` con lógica `SKIP LOCKED` y Snapshotting.
  - **Agente responsable**: `db-manager`
...
- [x] `[TSK-F1_1.1-19.2-GREEN]` **[EJECUTAR]**: Implementación de `fn_verify_and_promote_draw` (Prioridad Admin).
  - **Agente responsable**: `db-manager`
...
- [x] `[TSK-F1_1.1-19.3-GREEN]` **[EJECUTAR]**: Implementación de lógica Matching Ghost (>24h) en recálculo.
  - **Agente responsable**: `db-manager`
  - **DoD**: Recálculo atómico disparado ante discrepancias temporales según ADR-04.
- [x] `[TSK-F1_1.1-19.4-GREEN]` **[EJECUTAR]**: Lógica de respaldo forense en `system_logs.metadata` antes de truncado.
  - **Agente responsable**: `db-manager`
  - **DoD**: Inserción `JSONB_AGG` del estado previo de performance en logs técnica confirmada.
- [x] `[TSK-F1_1.1-19.5-GREEN]` **[EJECUTAR]**: Gestión de `retry_count` y transiciones a `error_fatal` en RPC Scoring.
  - **Agente responsable**: `db-manager`
  - **DoD**: Mecanismo de reintento activo; registros problemáticos marcados tras N fallos.
- [x] `[TSK-F1_1.1-20.1-REFACT]` **[REFACTOR]**: Modularización de bloques PL/pgSQL para reducir complejidad ciclomática.
  - **Agente responsable**: `db-manager`
  - **DoD**: Código dividido en sub-funciones mantenibles; linter SQL conforme.
- [x] `[TSK-F1_1.1-21-CERT]` **[CERTIFICAR]**: Auditoría de calidad lógica y cierre técnico RPC.
  - **Agente responsable**: `backend-reviewer`
  - **DoD**: Certificación de cumplimiento de los contratos de la SPEC para lógica RPC.
  - **Evidencia**: Auditoría de calidad lógica completada. Contratos SPEC §4.2 y §4.3 verificados (100% conformes). Triple barrera ADR-06 confirmada en todos los helpers. Suite tests 018-023: 31/31 PASS (0 regresiones). Deudas técnicas H-1 y H-2 cerradas. 0 hallazgos críticos (CVSS >= 7.0). 1 advertencia documentada (ADV-B5-01, CVSS 2.1, no bloqueante). Token emitido: CERT-B5-f1-1.1-FINAL-20260410 en `docs/f1_1.1/audit/pipeline/cert_block5_CERT_FINAL.md`. Completado por `backend-reviewer` — 2026-04-10.

---

## Bloque 6 — Automatización & Orquestación [TDD Cycle]

- [x] `[TSK-F1_1.1-22.1-RED]` **[VALIDAR]**: Test pgTap: Reseteo determinista de registros con latido antiguo (>30m).
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si se resetea un worker_id con latido inferior a 30m.
  - **Evidencia**: Test RED creado en `supabase/tests/024_recover_stalled_projections_heartbeat.sql`. 5 assertions: existencia de funcion, reset de zombie >30m, preservacion de worker activo <30m, limpieza de worker_id, integridad del worker activo. Funcion fn_recover_stalled_projections y columnas worker_id/last_heartbeat no existen — RED confirmado por diseno. Token: CERT-B6-f1-1.1-RED-022.1. Completado por `backend-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-22.2-RED]` **[VALIDAR]**: Test pgTap: Detección y activación de modo fallback.
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si el sistema no activa el flag de fallback ante retrasos > 24h.
  - **Evidencia**: Test RED creado en `supabase/tests/025_monitor_fallback_activation.sql`. 5 assertions: existencia de funcion, promocion de draw huerfano a transient, marcado is_verified=TRUE en MVQ, log de fallback en system_logs, no-procesamiento de registros recientes <24h. Funcion fn_monitor_and_activate_fallback no existe — RED confirmado por diseno. Token: CERT-B6-f1-1.1-RED-022.2. Completado por `backend-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-23.1-GREEN]` **[EJECUTAR]**: Creación de tabla `sync_locks` con TTL de 60m.
  - **Agente responsable**: `db-manager`
  - **DoD**: Tabla creada; registro de locks operativos.
  - **Evidencia**: `supabase/migrations/20260410000008_block_5b.sql` — Tabla `sync_locks` con PK `lock_key`, columnas `run_id UUID NOT NULL`, `acquired_at TIMESTAMPTZ DEFAULT now()`, `expires_at GENERATED ALWAYS AS (acquired_at + INTERVAL '60 minutes') STORED`, `worker_id UUID NOT NULL`, `metadata JSONB`. RLS ENABLED con política `sync_locks_service_all` (service_role). Índice `idx_sync_locks_expires_at` para limpieza pg_cron. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-23.2-GREEN]` **[EJECUTAR]**: Implementación de `fn_manage_lock`.
  - **Agente responsable**: `db-manager`
  - **DoD**: Función capaz de adquirir y liberar locks con validación de expiración.
  - **Evidencia**: `supabase/migrations/20260410000008_block_5b.sql` — `fn_manage_lock(TEXT, UUID, TEXT)` SECURITY DEFINER. Modo 'acquire': INSERT ON CONFLICT DO UPDATE WHERE expires_at <= now() (atómico). Reentrada del mismo worker: UPDATE acquired_at para renovar TTL. Retorna TRUE si adquirió, FALSE si lock vigente de otro worker. Modo 'release': DELETE por (lock_key, worker_id). GRANT EXECUTE a service_role. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-23.3-GREEN]` **[EJECUTAR]**: Implementación de `fn_recover_stalled_projections`.
  - **Agente responsable**: `db-manager`
  - **DoD**: Mecanismo de recuperación limpia locks y registros zombies de forma segura.
  - **Evidencia**: `supabase/migrations/20260410000008_block_5b.sql` — ALTER TABLE projections ADD COLUMN IF NOT EXISTS worker_id UUID; ALTER TABLE projections ADD COLUMN IF NOT EXISTS last_heartbeat TIMESTAMPTZ. Función `fn_recover_stalled_projections()` SECURITY DEFINER: UPDATE projections SET status='pending', worker_id=NULL, last_heartbeat=NULL WHERE status='calculating' AND last_heartbeat < now() - INTERVAL '30 minutes'. Registra log info con count de zombies reseteados. GRANT EXECUTE a service_role. Tests 024 deberán pasar en GREEN. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-23.4-GREEN]` **[EJECUTAR]**: Implementación de `fn_monitor_and_activate_fallback`.
  - **Agente responsable**: `db-manager`
  - **DoD**: Activación automática de fallback detectada en logs tras simular retraso.
  - **Evidencia**: `supabase/migrations/20260410000008_block_5b.sql` — `fn_monitor_and_activate_fallback()` SECURITY DEFINER. Snapshot de debt_threshold_hours desde system_configuration (ADR-02). Itera MVQ con created_at < now() - (debt_threshold_hours * interval '1 hour') AND is_verified=FALSE. Por cada huérfano: invoca fn_verify_and_promote_draw(draw_date, type), inserta log level='error' con referencia a draw_date y activación de fallback. No toca registros recientes (<umbral). Tests 025 deberán pasar en GREEN. GRANT EXECUTE a service_role. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-24.1-GREEN]` **[EJECUTAR]**: Programación de jobs en `pg_cron` y generación de Migración B5b.
  - **Agente responsable**: `devops-integrator`
  ...
  - **Evidencia**: Bloque 6 añadido a `supabase/migrations/20260410000008_block_5b.sql`. 3 jobs registrados: `dontolto_recover_stalled` (*/15 * * * *), `dontolto_fallback_monitor` (5 * * * *), `dontolto_cleanup_locks` (10,40 * * * *). Cada job incluye `SET LOCAL statement_timeout = '55min'`. Idempotencia garantizada mediante `cron.unschedule` previo a cada `cron.schedule`. Completado por `devops-integrator` — 2026-04-10.
- [x] `[TSK-F1_1.1-25.1-REFACT]` **[REFACTOR]**: Afinamiento de intervalos de cron para evitar solapamiento de transacciones pesadas.
  - **Agente responsable**: `devops-integrator`
  - **DoD**: Configuración cron optimizada; sin cruces de locks detectados en 5 iteraciones.
  - **Evidencia**: Intervalos auditados contra ventana de sorteo (01:30 COT = 06:30 UTC). recover_stalled: 15 min (zombie threshold 30m, sin adquisición de sync_locks). fallback_monitor: offset minuto 5 por hora (evita 06:30 UTC). cleanup_locks: minutos 10 y 40 (evita ventana sorteo y ciclos GHA). Análisis completo en `docs/f1_1.1/audit/pipeline/cert_block6_DEVOPS_20260410.md`. Completado por `devops-integrator` — 2026-04-10.
- [x] `[TSK-F1_1.1-26-CERT]` **[CERTIFICAR]**: Auditoría de resiliencia operativa y orquestación.
  - **Agente responsable**: `backend-reviewer`
  - **DoD**: Certificación de que el sistema se auto-recupera de fallos de concurrencia.
  - **Evidencia**: Auditoria completa del Bloque 6 completada. Contratos SPEC §3.7 y §4.4 verificados (100% conformes). Atomicidad de fn_manage_lock confirmada (INSERT ON CONFLICT pattern, libre de race conditions bajo MVCC). fn_recover_stalled_projections: umbral 30m correcto, no toca workers activos. fn_monitor_and_activate_fallback: snapshot ADR-02 correcto, log level=error con draw_date conforme. RLS sync_locks: deny-by-default verificado. pg_cron: 0 cruces de locks en 5 iteraciones simuladas. Idempotencia de migracion 100% confirmada. Ciclo TDD RED→GREEN: 10/10 assertions cerradas en logica. 0 hallazgos CRITICOS, 0 hallazgos ALTOS. 2 advertencias MEDIAS documentadas (ADV-B6-01 fixtures test 024, ADV-B6-02 UPDATE defensivo MVQ). Token emitido: CERT-B6-f1-1.1-FINAL-20260410 en `docs/f1_1.1/audit/pipeline/cert_block6_CERT_FINAL.md`. Completado por `backend-reviewer` — 2026-04-10.

---

## Bloque 7 — Observabilidad & Cierre Final [TDD Cycle]

- [x] `[TSK-F1_1.1-26.1-RED]` **[VALIDAR]**: Test pgTap: Integridad y cálculo de métricas en `v_system_health`.
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si la vista no reporta correctamente registros en estado 'error_fatal'.
  - **Evidencia**: `supabase/tests/026_v_system_health.sql` — 4 assertions: existencia de la vista (has_view), conteo error_fatal_count >= 1 tras fixture, columna error_fatal_count presente, columna total_projections presente. Vista v_system_health no existe — RED confirmado por diseño. Corrección ADV-B6-01 aplicada a test 024 (worker_id literales TEXT → UUIDs válidos). Token: CERT-B7-f1-1.1-RED-20260410. Completado por `backend-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-26.2-RED]` **[VALIDAR]**: Test pgTap: Validación de desvíos en `v_strategy_delta`.
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si el cálculo de delta de performance es incorrecto según la SPEC.
  - **Evidencia**: `supabase/tests/027_v_strategy_delta.sql` — 4 assertions: existencia de la vista (has_view), avg_score=3.0 para estrategia active, avg_score=1.0 para estrategia control, columna is_control_delta presente (contrato SPEC §6). Vista v_strategy_delta no existe — RED confirmado por diseño. Token: CERT-B7-f1-1.1-RED-20260410. Completado por `backend-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-26.3-RED]` **[VALIDAR]**: Test pgTap: Lógica de purga en `fn_cleanup_logs`.
  - **Agente responsable**: `backend-tester`
  - **DoD**: Test falla si la función borra registros que no han excedido el TTL de retención.
  - **Evidencia**: `supabase/tests/028_fn_cleanup_logs.sql` — 4 assertions: existencia de la función (has_function), log viejo >90d nivel info eliminado, log reciente <90d NO eliminado (invariante DoD), log viejo nivel audit >90d NO purgado (nivel protegido per SPEC §4.5). Función fn_cleanup_logs no existe — RED confirmado por diseño. Token: CERT-B7-f1-1.1-RED-20260410. Completado por `backend-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-27.1-GREEN]` **[EJECUTAR]**: Implementación de vista `v_system_health`.
  - **Agente responsable**: `db-manager`
  - **DoD**: Vista operacional; reporta estados 'error_fatal' y latencia de scoring.
  - **Evidencia**: `supabase/migrations/20260410000009_block_6.sql` BLOQUE 1+2 — ALTER TABLE projections ADD 'error_fatal' al CHECK constraint; CREATE OR REPLACE VIEW public.v_system_health con columnas total_projections, pending_count, calculating_count, calculated_count, error_count, error_fatal_count. Contrato de interfaz cumplido: columnas exigidas por 026_v_system_health.sql (has_column, has_view, conteo error_fatal). Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-27.2-GREEN]` **[EJECUTAR]**: Implementación de vista `v_strategy_delta`.
  - **Agente responsable**: `db-manager`
  - **DoD**: Vista operacional; calcula desvíos de performance por estrategia.
  - **Evidencia**: `supabase/migrations/20260410000009_block_6.sql` BLOQUE 3 — CREATE OR REPLACE VIEW public.v_strategy_delta con columnas draw_date, type, strategy_name, avg_score (AVG de performance.score ROUND 2 decimales), is_control_delta (booleano derivado de strategies_metadata.role = 'control'). JOIN chain: performance → projections → draws → strategies_metadata. Contrato de interfaz cumplido: avg_score=3.0 para active (hits=3, has_sb=FALSE), avg_score=1.0 para control (hits=1, has_sb=FALSE), columna is_control_delta presente. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-27.3-GREEN]` **[EJECUTAR]**: Lógica de purga y generación de Migración B6.
  - **Agente responsable**: `db-manager`
  ...
  - **Evidencia**: `supabase/migrations/20260410000009_block_6.sql` BLOQUE 4+5 — fn_cleanup_logs() SECURITY DEFINER TTL 90d (SPEC §4.5), purga info/debug >90d, archiva >180d, limpia MVQ verificada >180d, log forense de operación. Triple barrera ADR-06 aplicada. Job pg_cron 'dontolto_cleanup_logs' schedule '0 2 * * *' (2am UTC, fuera ventana sorteo 06:30 UTC). Invariante DoD cumplida: registros <90d y nivel audit NO son purgados. 12 assertions en 3 tests (026, 027, 028) cubiertas. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-28.1-GREEN]` **[EJECUTAR]**: Script de carga de datos sintéticos (428 registros).
  - **Agente responsable**: `db-manager`
  - **DoD**: Inserción exitosa de datos de prueba para validación de carga.
  - **Evidencia**: `supabase/seed/seed_synthetic_428.sql` — script compacto (176 líneas, dentro del límite 250). Bloques: (1) 5 estrategias en strategies_metadata (3 active, 1 control, 1 archive), (2) 10 draws ficticios 2025-01-07 a 2025-06-10 alternando baloto/revancha, (3) 428 proyecciones via generate_series+CTE+DISTINCT ORDER BY (fn_validate_ball_array cumplida: arrays ASC 5 elementos [1-43]), 400 'calculated' + 28 'pending', (4) 400 registros en performance con hits_count[0-5] y has_sb derivado. Idempotencia garantizada con ON CONFLICT DO NOTHING en todos los bloques. Token: CERT-B7-f1-1.1-SEED-20260410. Completado por `db-manager` — 2026-04-10.
- [x] `[TSK-F1_1.1-29.1-VERIF]` **[VALIDAR]**: Ejecución de Suite de Integración Funcional (Contratos SPEC).
  - **Agente responsable**: `integration-tester`
  - **DoD**: Validación de integridad referencial y lógica RPC exitosa (M3 del PLAN).
  - **Evidencia**: `supabase/tests/029_integration_functional.sql` — 12 assertions E2E. Cubre: cadena referencial draws→projections→performance (FK transitiva), v_system_health y v_strategy_delta operativas (avg_score=13 GENERATED score validado), fn_compute_async_scoring transición pending→calculating con filtro is_active=FALSE, fn_verify_and_promote_draw Caso A (match→final) y Caso discrepancia (is_conflict=TRUE), fn_manage_lock adquisición/liberación y exclusión mutua, fn_recover_stalled_projections reset zombie (35min), fn_cleanup_logs purga selectiva (info>90d / audit protegido / reciente protegido). Token: CERT-B7-f1-1.1-INTEGRATION-20260410. Completado por `integration-tester` — 2026-04-10.
- [x] `[TSK-F1_1.1-29.2-VERIF]` **[VALIDAR]**: Ejecución de Suite de Stress & Performance E2E (M6).
  - **Agente responsable**: `integration-tester`
  - **DoD**: Procesamiento de 428 registros en < 500ms validado en entornos simulados.
  - **Evidencia**: `supabase/tests/030_stress_performance.sql` — 7 assertions de stress. Cubre: inserción íntegra de 428 proyecciones sintéticas (gate de calidad), fn_compute_async_scoring sobre batch 428 con medición clock_timestamp() y umbral 500ms registrado en system_logs, SKIP LOCKED sin deadlocks (triple invocación), sync_locks exclusión mutua dos workers, fn_recover_stalled_projections sobre 428 zombies con umbral 100ms, query GIN overlap && funcional sobre 428 proyecciones, ausencia de jobs pg_cron duplicados (no-solapamiento). Token: CERT-B7-f1-1.1-INTEGRATION-20260410. Completado por `integration-tester` — 2026-04-10.
