...
-- Tabla Singleton de configuración administrativa
...
COMMENT ON TABLE public.system_configuration IS
    'Singleton de configuración administrativa del sistema. Solo puede existir 1 registro (id=1). Garantía: CHECK (id = 1) + trigger de bloqueo de DELETE.';
...
-- =============================================================================
-- TSK-F1_1.1-05.2-GREEN — Trigger: Bloqueo de DELETE en system_configuration
-- SPEC: §3.6 — Garantía Singleton
-- =============================================================================

-- Función trigger que bloquea TODO intento de DELETE
...
-- Trigger BEFORE DELETE — aplica a cualquier DELETE, con o sin WHERE
-- FOR EACH STATEMENT: intercepta la sentencia completa incluso si la tabla está vacía,
-- garantizando que no haya elusión via DELETE sobre tabla sin filas.
...
COMMENT ON TABLE public.system_configuration IS
    'Singleton de configuración administrativa. id=1 es el único registro permitido. Protegido por CHECK(id=1) y trigger tg_prevent_singleton_delete.';
...
CREATE TABLE IF NOT EXISTS public.draws (
    ...
    numbers     INTEGER[]   NOT NULL
                CONSTRAINT chk_draws_numbers CHECK (
                    array_length(numbers, 1) = 5
                    AND numbers[1] >= 1 AND numbers[5] <= 43
...
-- Índice único para garantizar un sorteo por fecha y tipo
...
COMMENT ON TABLE public.draws IS
    'Historial oficial de sorteos Baloto/Revancha. Unicidad garantizada por (draw_date, type). Constraint de array: orden ASC estricto, rango [1-43], cardinalidad=5.';
...
CREATE TABLE IF NOT EXISTS public.manual_verification_queue (
    ...
    draw_date    DATE        NOT NULL,
    numbers      INTEGER[]   NOT NULL
                 CONSTRAINT chk_mvq_numbers CHECK (
                     array_length(numbers, 1) = 5
                     AND numbers[1] >= 1 AND numbers[5] <= 43
                     AND numbers[1] < numbers[2]
                     AND numbers[2] < numbers[3]
...
-- Índice para búsquedas por fecha y tipo (relación lógica con draws)
CREATE INDEX IF NOT EXISTS idx_mvq_draw_date_type
    ON public.manual_verification_queue (draw_date, type);

-- Índice para filtrar registros no verificados (query frecuente en fn_monitor_and_activate_fallback)
...
-- =============================================================================
-- TSK-F1_1.1-05.6-GREEN — Upgrade CHECK draws.numbers → fn_validate_ball_array
-- =============================================================================

-- Eliminar el CHECK inline temporal
ALTER TABLE public.draws
    DROP CONSTRAINT IF EXISTS chk_draws_numbers;

-- Reemplazar con la función IMMUTABLE
ALTER TABLE public.draws
    ADD CONSTRAINT chk_draws_numbers
    CHECK (public.fn_validate_ball_array(numbers));

-- Mismo upgrade para manual_verification_queue
ALTER TABLE public.manual_verification_queue
    DROP CONSTRAINT IF EXISTS chk_mvq_numbers;

ALTER TABLE public.manual_verification_queue
    ADD CONSTRAINT chk_mvq_numbers
    CHECK (public.fn_validate_ball_array(numbers));
...
-- IMPORTANTE: Este INSERT solo se ejecuta si no existe ya el registro (idempotencia)
-- =============================================================================
